...
void PlayerElements::InputPlayer(std::unordered_map<std::pair<std::string, std::string>, std::string> A) { // Llegeix l'Input de l'usuari 
	...
	else if ((str[0] >= '0' && str[0] <= '9') && (str2[0] >= '0' && str2[0] <= '9'))				// Si no es cap de les anterior y el primer element dels 2 strings es un numero fa un cast
	{
		...
		else if (a == b){																			// Si a és igual a b no et deixa fer la combinació
			system("cls");
...
bool PlayerElements::getBool(bool &EndGame) {														// Fem un bool per a la puntuació
	...
	else																							// El joc acaba quan és igual a 390 combinacions
	{
		EndGame = false;
	}
	return EndGame;
